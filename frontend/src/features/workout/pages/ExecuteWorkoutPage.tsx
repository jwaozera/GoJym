...
type SummaryMode = 'completed' | 'partial'
...
export const ExecuteWorkoutPage = () => {
  ...
  useEffect(() => {
    ...
    const loadExerciseReferences = async () => {
      const references = await Promise.all(
        uniqueIds.map(async (exercicioId) => ({
          exercicioId,
          record: await workoutService.getExerciseRecord(exercicioId),
          previousSets: await workoutService.getLastSessionSeriesByExercise(exercicioId),
        }))
      )
      ...
      const recordMap = new Map(
        references.map(({ exercicioId, record }) => [exercicioId, record?.maiorCarga ?? null])
      )

      const previousSetsMap = new Map(
        references.map(({ exercicioId, previousSets }) => [
          exercicioId,
          new Map(previousSets.map((set) => [set.setNumber, set])),
        ])
      )
      ...
      setExercises((prev) =>
        prev.map((exercise) => ({
          ...
          sets: exercise.sets.map((set) => {
            const previousSet =
              exercise.exercicioId !== null
                ? previousSetsMap.get(exercise.exercicioId)?.get(set.setNumber)
                : undefined
  ...
  if (view === 'summary') {
    ...
    const maxWeightSummary = maxWeightSet.weight > 0
      ? `${maxWeightSet.weight}kg — ${maxExName}`
      : '—'
    const isPartialSummary = summaryMode === 'partial'
      ...
      >
        ...
        <div className="flex flex-col items-center pt-16 pb-4">
          <div
            ...
            style={{
              background: isPartialSummary
                ? 'linear-gradient(135deg, #F59E0B, #D97706)'
                : 'linear-gradient(135deg, #10B981, #059669)',
              boxShadow: isPartialSummary
          ...
          <h1 className="text-2xl font-bold text-white">
            {isPartialSummary ? 'Sessão encerrada' : 'Treino concluído'}
          </h1>
          {isPartialSummary && (
            <p className="mt-1 text-xs text-gj-text-secondary">
              Resumo do que foi executado até agora
            </p>
          )}
        </div>
          ...
          <div className="mx-5 mb-4 rounded-gj-lg bg-gj-surface border border-gj-border overflow-hidden">
            ...
            <div className="flex flex-col gap-2 p-3">
              {summaryHighlights.map((highlight) => (
                <div
                  key={highlight.type}
                  className={`flex items-center gap-3 p-3 rounded-gj-md border ${highlight.className}`}
                >
                  {highlight.icon}
                  <div>
                    <span className="text-xs font-semibold text-white block">{highlight.title}</span>
